...
| `test_build.py` | Checks `build.py` against figures the spreadsheet published independently |
...
Open `index.html` in a browser to run it. The page itself has no dependencies.

The scripts need **Python 3.8 or newer and nothing else** — standard library only, no pip install.
...
Both tasks below end the same way: run `build.py`, then `test_build.py`. `build.py` refuses to write if a price carries an incomplete provenance record, so read this section before editing, or the build will reject the row.
